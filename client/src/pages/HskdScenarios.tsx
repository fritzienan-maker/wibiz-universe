      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-6">
        <div className="max-w-lg w-full bg-card border border-destructive/30 rounded-2xl p-8 space-y-4 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Your response has been flagged. WiBiz Ops will contact you within 1 business day.
          </p>
          <Link to="/dashboard" className="inline-block mt-2 text-sm px-4 py-2 rounded-lg bg-muted text-muted-foreground hover:text-foreground transition-colors">
            Return to Dashboard
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="bg-card border-b border-border px-6 py-4 flex justify-between items-center">
        ...
        <span className="text-sm text-muted-foreground">Scenario {current + 1} of {scenarios.length}</span>
      </header>

      <main className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <div className="w-full bg-muted rounded-full h-1.5">
          <div className="bg-primary h-1.5 rounded-full transition-all duration-500" style={{ width: `${(current / scenarios.length) * 100}%` }} />
        </div>
