...
hskdClientRouter.post("/start", async (req: Request, res: Response): Promise<void> => {
  console.log("[hskd] start body:", JSON.stringify(req.body));
  ...
  try {
    ...
    if (existing.rows.length) {
      const existingCert = existing.rows[0] as any;
      if (existingCert.status === "TRAINING") {
        // Clear stuck TRAINING certification and allow restart
        await pool.query(`DELETE FROM client_certifications WHERE id = $1`, [existingCert.id]);
      } else {
        res.status(409).json({
          error: "You already have an active certification in progress",
          certification_id: existingCert.id,
        });
        return;
      }
    }
